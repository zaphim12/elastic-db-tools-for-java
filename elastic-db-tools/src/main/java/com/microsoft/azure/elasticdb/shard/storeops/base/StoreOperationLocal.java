...
public abstract class StoreOperationLocal implements IStoreOperationLocal, AutoCloseable {
     ...
     *
     * @param credentials
     *            Credentials for connecting to SMM databases.
     * @param retryPolicy
     *            Retry policy for requests.
     * @param location
     *            Shard location where the operation is to be performed.
     * @param operationName
     *            Operation name.
     */
    public StoreOperationLocal(SqlShardMapManagerCredentials credentials,
    ...
    public ShardManagementException onStoreException(StoreException se) {
        return ExceptionUtils.getStoreExceptionLocal(ShardManagementErrorCategory.Recovery, se, this.getOperationName(), this.getLocation());
    }
    ...
    private void establishConnection() {
        ...
        localConnection = new SqlStoreConnection(StoreConnectionKind.LocalSource, localConnectionString.getConnectionString());
    }
    ...
    private IStoreTransactionScope getTransactionScope() {
        return localConnection.getTransactionScope(this.getReadOnly() ? StoreTransactionScopeKind.ReadOnly : StoreTransactionScopeKind.ReadWrite);
    }
